...
!-----------------!
 ...
 real(8) :: alpha,dt,t,dx,x0,N,integral
 ...
 call initpsi(npoints,dx,psi0)                       !Obtain initial wavepacket psi0
...
!--------------------------------------------!
 subroutine initpsi(npoints,dx,psi0)
!--------------------------------------------!
 use parameters
 implicit none

 integer :: i,j,npoints,n
 real(8) :: x, dx, integral, Normal, psi_n
 real(8) :: c(0:4) = (/1.0d0, 0.0d0, 1.0d0, 0.0d0, 1.0d0/)
 ...
 do i=-npoints/2+1,npoints/2
    ...
    ! composing the wavepacket using the eigenstate subroutine 
    do n = 0, 4
       call eigenstate(n, x, psi_n)
       psi0(j) = psi0(j) + c(n) * psi_n
    end do
  end do
 ...
 integral = 0.0
 do i = 1, npoints
  integral = integral + abs(psi0(i)**2)*dx
 end do

 Normal = 1.0d0/sqrt(integral)

 do i = 1, npoints
   psi0(i) = Normal * psi0(i)
 end do
...
!-----------------------------------!
subroutine hermite_poly(n,y,Hn)
!-----------------------------------!
        implicit none
        integer, intent(in)   :: n
        real(8), intent(in)   :: y
        real(8), intent(out)  :: Hn
        real(8)               :: Hnm1,Hnm2, Hn_temp
        integer               :: i
        ...
        if (n == 0) then
                ...
                ! Start recursion with H0 and H1
                Hnm2 = 1.0d0
                Hnm1 = 2.0d0 * y
                
                ! Perform recursion up to n
                do i = 2, n 
                   Hn_temp = 2.d0 * y * Hnm1 - 2.d0 * (i-1) * Hnm2
                   Hnm2 = Hnm1
                   Hnm1 = Hn_temp
                end do
...
!-----------------------------------!
subroutine eigenstate(n, x, psi_n)
!-----------------------------------!
        use parameters
        implicit none 
        integer, intent(in)    :: n 
        real(8), intent(in)    :: x 
        real(8), intent(out)   :: psi_n
        real(8)                :: y, Hn, norm_factor
        integer                :: factorial 
        
        ! Calculating the hermite polynomials
        y = sqrt(mass * angfreq) * x
        call hermite_poly(n , y, Hn)
        
        ! Calculating the eigen states
        norm_factor = 1.0d0 / sqrt(2.0d0**n * factorial (n)) * (mass * angfreq/pi) ** 0.25d0
        psi_n = norm_factor * exp(-0.5d0 * mass * angfreq * x**2) * Hn

end subroutine eigenstate 
!------------------------!
